...
StAmsdos * CreeEnteteAmsdos( char * NomFic, unsigned short Longueur ) {
    ...
    Entete.Length = 0; 	//Not set by AMSDos !!
    Entete.RealLength = Entete.LogicalLength = Longueur;
    Entete.FileType = 2; //Binary file
...
//
// Returns the position of a sector in the DSK file
//
int DSK::GetPosData( int track, int sect, bool SectPhysique ) {
    // Search sector position
...
//
// Write "raw" data into the floppy image
//
...
//
// Read an AMSDOS block (1 block = 2 sectors)
//
...
//
// Fill a "bitmap" to know where files are located on the floppy disk
// Also returns the number of KB used on the disk
//
...
//
// Returns the free space available on the disk in KB
//
int DSK::GetFreeSpace( void ) {
    int usedKB = FillBitmap();
    return 178 - usedKB;  // 178 KB is the standard CPC disk capacity
}
...
//
// Check file existence, returns file index if it exists,
// -1 otherwise
//
	...
	bool System_file, bool Read_only )
{
	...
	if ((Hfile = fopen(Masque.c_str(),"rb")) == NULL) {
		cerr << "Error: Could not open file " << Masque << endl;
		return false;
	}
	...
	switch(TypeModeImport) {
		case MODE_ASCII:
			...
			// Convert Unix line endings (\n) to DOS line endings (\r\n) for BASIC files only
			{
				// Check if file has .bas extension
				string filename = Masque;
				bool isBasFile = false;
				...
				if (filename.length() >= 4) {
					...
					isBasFile = (extension == ".bas");
				}
				
				if (isBasFile) {
					static unsigned char TempBuff[0x20000];
					unsigned long NewLg = 0;
					
					for (unsigned long i = 0; i < Lg; i++) {
						if (Buff[i] == '\n' && (i == 0 || Buff[i-1] != '\r')) {
							// Add \r before \n if not already there
							TempBuff[NewLg++] = '\r';
							TempBuff[NewLg++] = '\n';
						} else {
							TempBuff[NewLg++] = Buff[i];
						}
					}
					
					// Copy back to original buffer
					memcpy(Buff, TempBuff, NewLg);
					Lg = NewLg;
					cout << "Converted Unix line endings to DOS format for BASIC file\n";
				}
			}
		break;
...
bool DSK::Hexdecimal() {
	...
	while (TailleCourante <=  TailleFic ) {
		// display the offset 
		memset( OffSet, 0 , 7 );
		snprintf( OffSet, 7, "#%.4X:", TailleCourante );
