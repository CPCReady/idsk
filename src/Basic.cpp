...
void Basic( BYTE * BufFile, char * Listing, bool IsBasic, bool CrLf )
{
    ...
    * Listing = 0;
    ...
    for ( ;; )
                        ...
                        if ( Token >= 0x20 && Token < 0x7C )
                            ...
                            switch( Token )
                                ...
                                case 0x19 : // Constante entière 8 bits
                                    snprintf(Tmp, 32, "%d",(BYTE)GetByte( BufFile, Pos, Deprotect));
                                    strcat(Listing, Tmp);
                                    Pos++;
                                    break;
                                ...
                                case 0x1E : // Constante entière 16 bits
                                    snprintf(Tmp, 32, "%d", GetWord( BufFile, Pos, Deprotect));
                                    strcat(Listing, Tmp);
                                    Pos += 2;
                                    break;
                                ...
                                case 0x1F : // Constante flottante
                                    ...
                                    Pos += 5;
                                    snprintf( Tmp, 32, "%f", f * pow( (double) 2, exp ) );
        ...
        // Always add newline after each BASIC line for better readability
        int currentLen = strlen(Listing);
        Listing[currentLen] = '\n';
        Listing[currentLen + 1] = '\0';
